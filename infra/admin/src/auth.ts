...
const KEY_ID_TOKEN  = 'pai_admin_id_token';
const KEY_ACCESS    = 'pai_admin_access_token';
const KEY_VERIFIER  = 'pai_admin_pkce_verifier';
const KEY_STATE     = 'pai_admin_pkce_state';
...
export function getUser(): AdminUser | null {
  ...
  try {
    ...
    if (Number(payload.exp) * 1000 < Date.now()) { logout(); return null; }
    return {
      sub:      payload.sub ?? '',
      email:    payload.email ?? '',
